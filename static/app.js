...
function initMusicStep() {
  ...
  // Build the UI for user choice
  container.innerHTML = `
    <p class="card-subtitle" style="margin-bottom: 1rem;">How do you want to choose the background music?</p>
    
    <div class="actions-row" style="margin-bottom: 2rem;">
      <button class="btn btn-primary" onclick="startMusicLoading(null)" style="width: 100%; padding: 1rem; font-size: 1.1rem;">
        ✨ Let AI Auto-Suggest based on story
      </button>
    </div>

    <p class="card-subtitle" style="text-align: center; margin-bottom: 1rem;">OR choose a vibe yourself:</p>
    
    <div class="lang-grid" id="vibe-grid" style="margin-bottom: 1rem;">
      ${VIBES.map(v => `
        <div class="lang-chip" onclick="startMusicLoading('${v.id}')" style="cursor: pointer; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
...
// Helper function to transition from the menu to the loading screen
window.startMusicLoading = function(vibe) {
  const container = $('#step-3-content');
  const message = vibe 
    ? `Generating ${vibe} music...` 
    : `AI is analyzing the story to pick the perfect music vibe...`;
    
  container.innerHTML = `
    <div class="status-pill"><span class="dot"></span> ${message}</div>
  `;
  
  // Send the request to your backend (null vibe triggers Gemini)
  generateMusic("ai", vibe);
}

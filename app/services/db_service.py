...
def create_project(mode: str, language: str, prompt: str = None) -> str:
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO projects (id, mode, prompt, language, status) VALUES (%s, %s, %s, %s, %s)",
            (project_id, mode, prompt, language, "uploaded")
        )
        conn.commit()
    finally:
        conn.close()
